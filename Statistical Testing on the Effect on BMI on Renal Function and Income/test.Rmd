...
df$Income_Factor <- cut(df$HHIncomeMid,breaks = c(min(df$HHIncomeMid)-1,22500,87500,max(df$HHIncomeMid)),labels = c("Low","Medium","High"),include.lowest = TRUE)
...
```
The Summary Statistics of the Population:
```{r 5,results='asis',message=FALSE,warning=FALSE}
kable(summarize(df))
kable(summarize(df, type = "factor",cumulative=TRUE))
```
The Summary Statistics of the Sample:
```{r 7,results='asis',message=FALSE,warning=FALSE}
kable(summarize(df1))
kable(summarize(df1, type = "factor",cumulative=TRUE))
```
